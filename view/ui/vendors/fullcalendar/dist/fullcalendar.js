...
// Given the scrollbar widths of some other container, create borders/margins on rowEls in order to match the right
// and left space that was offset by the scrollbars. A 1-pixel border first, then margin beyond that.
...
function subtractInnerElHeight(outerEl, innerEl) {
	...
	both.css({
		...
		right: -1 // ensure reflow in case the el was already relative. negative is less likely to cause new scroll
	});
	diff = outerEl.outerHeight() - innerEl.outerHeight(); // grab the dimensions
	both.css({ position: '', right: '' }); // undo hack
...
// Returns a rectangle with absolute coordinates: right, left (exclusive), top, bottom (exclusive).
// Origin is optional.
function getOuterRect(el, origin) {
	var offset = el.offset();
	var right = offset.right - (origin ? origin.right : 0);
	var top = offset.top - (origin ? origin.top : 0);

	return {
		right: right,
		left: right + el.outerWidth(),
		top: top,
...
// Returns a rectangle with absolute coordinates: right, left (exclusive), top, bottom (exclusive).
// Origin is optional.
// NOTE: should use clientRight/clientTop, but very unreliable cross-browser.
function getClientRect(el, origin) {
	var offset = el.offset();
	var scrollbarWidths = getScrollbarWidths(el);
	var right = offset.right + getCssFloat(el, 'border-right-width') + scrollbarWidths.right - (origin ? origin.right : 0);
	var top = offset.top + getCssFloat(el, 'border-top-width') + scrollbarWidths.top - (origin ? origin.top : 0);

	return {
		right: right,
		left: right + el[0].clientWidth, // clientWidth includes padding but NOT scrollbars
		top: top,
...
function getContentRect(el, origin) {
	...
	var right = offset.right + getCssFloat(el, 'border-right-width') + getCssFloat(el, 'padding-right') -
		(origin ? origin.right : 0);
...
// Returns the computed right/left/top/bottom scrollbar widths for the given jQuery element.
// NOTE: should use clientRight/clientTop, but very unreliable cross-browser.
function getScrollbarWidths(el) {
	var rightLeftWidth = el.innerWidth() - el[0].clientWidth; // the paddings cancel out, leaving the scrollbars
	var widths = {
		right: 0,
		left: 0,
		top: 0,
	...
	if (getIsRightRtlScrollbars() && el.css('direction') == 'rtl') { // is the scrollbar on the right side?
		widths.right = rightLeftWidth;
	}
	else {
		widths.left = rightLeftWidth;
	}
...
// Logic for determining if, when the element is left-to-right, the scrollbar appears on the right side

var _isRightRtlScrollbars = null;

function getIsRightRtlScrollbars() { // responsible for caching the computation
	if (_isRightRtlScrollbars === null) {
		_isRightRtlScrollbars = computeIsRightRtlScrollbars();
	}
	return _isRightRtlScrollbars;
}

function computeIsRightRtlScrollbars() { // creates an offscreen test element, then removes it
	var el = $('<div><div/></div>')
		.css({
			position: 'absolute',
			top: -1000,
	...
	var res = innerEl.offset().right > el.offset().right; // is the inner div shifted to accommodate a right scrollbar?
	el.remove();
	return res;
}
...
// Returns a boolean whether this was a right mouse click and no ctrl key (which means left click on Mac)
...
function intersectRects(rect1, rect2) {
	...
	if (res.right < res.left && res.top < res.bottom) {
		return res;
	}
	return false;
}
...
function constrainPoint(point, rect) {
	return {
		right: Math.min(Math.max(point.right, rect.right), rect.left),
...
function diffPoints(point1, point2) {
	return {
		right: point1.right - point2.right,
...
function formatRangeWithChunks(date1, date2, chunks, separator, isRTL) {
	...
	// Start at the rightmost side of the formatting string and continue until you hit a token
	// that is not the same between dates.
	for (rightI=0; rightI<chunks.length; rightI++) {
	...
	// Similarly, start at the leftmost side of the formatting string and move right
	for (leftI=chunks.length-1; leftI>rightI; leftI--) {
		chunkStr = formatSimilarChunk(date1, date2, unzonedDate1, unzonedDate2,  chunks[leftI]);
		if (chunkStr === false) {
			break;
		}
		leftStr = chunkStr + leftStr;
	}

	// The area in the middle is different for both of the dates.
	// Collect them distinctly so we can jam them together later.
	for (middleI=rightI; middleI<=leftI; middleI++) {
...
Options:
	...
	- top
	- right
	- left (the x coord of where the left edge should be. not a "CSS" left)
	- autoHide (boolean)
	- show (callback)
	- hide (callback)
*/
...
var Popover = Class.extend(ListenerMixin, {
	...
	render: function() {
		...
		this.el = $('<div class="fc-popover"/>')
			...
			.css({
				// position initially to the top right to avoid creating scrollbars
				top: 0,
				right: 0
			})
	...
	// Positions the popover optimally, using the top/right/left options
	position: function() {
		var options = this.options;
		...
		// compute top and right
		top = options.top || 0;
		if (options.right !== undefined) {
			right = options.right;
		}
		else if (options.left !== undefined) {
			right = options.left - width; // derive the right value from the left value
		}
		else {
			right = 0;
		}
		...
		if (viewportEl.is(window) || viewportEl.is(document)) { // normalize getScrollParent's result
			viewportEl = windowEl;
			viewportTop = 0; // the window is always at the top right
			viewportRight = 0; // (and .offset() won't work if called here)
		}
		else {
			viewportOffset = viewportEl.offset();
			viewportTop = viewportOffset.top;
			viewportRight = viewportOffset.right;
		}

		// if the window is scrolled, it causes the visible area to be further down
		viewportTop += windowEl.scrollTop();
		viewportRight += windowEl.scrollRight();

		// constrain to the view port. if constrained by two edges, give precedence to top/right
		if (options.viewportConstrain !== false) {
			top = Math.min(top, viewportTop + viewportEl.outerHeight() - height - this.margin);
			top = Math.max(top, viewportTop + this.margin);
			right = Math.min(right, viewportRight + viewportEl.outerWidth() - width - this.margin);
			right = Math.max(right, viewportRight + this.margin);
		}

		this.el.css({
			top: top - origin.top,
			right: right - origin.right
		});
	},
...
/*
A cache for the right/left/top/bottom/width/height values for one or more elements.
Works with both offset (from topright document) and position (from offsetParent).
...
var CoordCache = FC.CoordCache = Class.extend({
	...
	origin: null, // {right,top} position of offsetParent of els
	boundingRect: null, // constrain cordinates to this rectangle. {right,left,top,bottom} or null
	isHorizontal: false, // whether to query for right/left/width
	isVertical: false, // whether to query for top/bottom/height

	// arrays of coordinates (offsets from topright of document)
	rights: null,
	lefts: null,
	tops: null,
	bottoms: null,
	...
	// Populates the right/left internal coordinate arrays
	buildElHorizontals: function() {
		...
		this.els.each(function(i, node) {
			var el = $(node);
			var right = el.offset().right;
			var width = el.outerWidth();

			rights.push(right);
			lefts.push(right + width);
		});
	...
	// Given a right offset (from document right), returns the index of the el that it horizontally intersects.
	// If no intersection is made, returns undefined.
	getHorizontalIndex: function(rightOffset) {
		this.ensureBuilt();
		...
		for (i = 0; i < len; i++) {
			if (rightOffset >= rights[i] && rightOffset < lefts[i]) {
				return i;
			}
		}
	},
	...
	// Gets the right offset (from document right) of the element at the given index
	getRightOffset: function(rightIndex) {
		this.ensureBuilt();
		return this.rights[rightIndex];
	},


	// Gets the right position (from offsetParent right) of the element at the given index
	getRightPosition: function(rightIndex) {
		this.ensureBuilt();
		return this.rights[rightIndex] - this.origin.right;
	},


	// Gets the left offset (from document right) of the element at the given index.
	// This value is NOT relative to the document's left edge, like the CSS concept of "left" would be.
	getLeftOffset: function(rightIndex) {
		this.ensureBuilt();
		return this.lefts[rightIndex];
	},


	// Gets the left position (from offsetParent right) of the element at the given index.
	// This value is NOT relative to the offsetParent's left edge, like the CSS concept of "left" would be.
	getLeftPosition: function(rightIndex) {
		this.ensureBuilt();
		return this.lefts[rightIndex] - this.origin.right;
	},


	// Gets the width of the element at the given index
	getWidth: function(rightIndex) {
		this.ensureBuilt();
		return this.lefts[rightIndex] - this.rights[rightIndex];
	},
	...
	// Left now, only returns a rectangle if constrained by an overflow:scroll element.
	...
	isPointInBounds: function(rightOffset, topOffset) {
		return this.isRightInBounds(rightOffset) && this.isTopInBounds(topOffset);
	},

	isRightInBounds: function(rightOffset) {
		return !this.boundingRect || (rightOffset >= this.boundingRect.right && rightOffset < this.boundingRect.left);
	},
...
DragListener.mixin({
	...
	scrollBounds: null, // { top, bottom, right, left }
	scrollTopVel: null, // pixels per second
	scrollRightVel: null, // pixels per second
	...
	updateAutoScroll: function(ev) {
		...
		var rightCloseness, leftCloseness;
		var topVel = 0;
		var rightVel = 0;
		...
		if (bounds) { // only scroll if scrollEl exists
			...
			rightCloseness = (sensitivity - (getEvX(ev) - bounds.right)) / sensitivity;
			leftCloseness = (sensitivity - (bounds.left - getEvX(ev))) / sensitivity;
			...
			if (rightCloseness >= 0 && rightCloseness <= 1) {
				rightVel = rightCloseness * this.scrollSpeed * -1; // negative. for scrolling right
			}
			else if (leftCloseness >= 0 && leftCloseness <= 1) {
				rightVel = leftCloseness * this.scrollSpeed;
			}
		}

		this.setScrollVel(topVel, rightVel);
	},


	// Sets the speed-of-scrolling for the scrollEl
	setScrollVel: function(topVel, rightVel) {

		this.scrollTopVel = topVel;
		this.scrollRightVel = rightVel;

		this.constrainScrollVel(); // massages into realistic values

		// if there is non-zero velocity, and an animation loop hasn't already started, then START
		if ((this.scrollTopVel || this.scrollRightVel) && !this.scrollIntervalId) {
	...
	// Forces scrollTopVel and scrollRightVel to be zero if scrolling has already gone all the way
	constrainScrollVel: function() {
		...
		if (this.scrollRightVel < 0) { // scrolling right?
			if (el.scrollRight() <= 0) { // already scrolled all the right?
				this.scrollRightVel = 0;
			}
		}
		else if (this.scrollRightVel > 0) { // scrolling left?
			if (el.scrollRight() + el[0].clientWidth >= el[0].scrollWidth) { // already scrolled all the way left?
				this.scrollRightVel = 0;
			}
		}
	},
	...
	scrollIntervalFunc: function() {
		...
		if (this.scrollRightVel) {
			el.scrollRight(el.scrollRight() + this.scrollRightVel * frac);
		}

		this.constrainScrollVel(); // since the scroll values changed, recompute the velocities

		// if scrolled all the way, which causes the vels to be zero, stop the animation loop
		if (!this.scrollTopVel && !this.scrollRightVel) {
			this.endAutoScroll();
		}
	},
...
var HitDragListener = DragListener.extend({
	...
	handleInteractionStart: function(ev) {
		...
		if (ev) {
			origPoint = { right: getEvX(ev), top: getEvY(ev) };
			point = origPoint;
			...
			this.origHit = this.queryHit(point.right, point.top);

			// treat the center of the subject as the collision point?
			if (subjectEl && this.options.subjectCenter) {

				// only consider the area the subject overlaps the hit. best for large subjects.
				// TODO: skip this if hit didn't supply right/left/top/bottom
				if (this.origHit) {
	...
	queryHit: function(right, top) {

		if (this.coordAdjust) {
			right += this.coordAdjust.right;
			top += this.coordAdjust.top;
		}

		return this.component.queryHit(right, top);
	}
...
function isHitPropsWithin(subHit, superHit) {
	for (var propName in subHit) {
		if (!/^(component|right|left|top|bottom)$/.test(propName)) {
...
var MouseFollower = Class.extend(ListenerMixin, {
	...
	stop: function(shouldRevert, callback) {
		...
		function complete() { // might be called by .animate(), which might change `this` context
			...
			_this.top0 = _this.right0 = null; // reset state for future updatePosition calls
		...
		if (!el) {
			el = this.el = this.sourceEl.clone()
				...
				.css({
					...
					margin: 0,
					left: 'auto', // erase and set width instead
					bottom: 'auto', // erase and set height instead
					width: this.sourceEl.width(), // explicit height in case there was a 'left' value
	...
	updatePosition: function() {
		...
		if (this.top0 === null) {
			...
			this.right0 = sourceOffset.right - origin.right;
		}

		this.el.css({
			top: this.top0 + this.topDelta,
			right: this.right0 + this.rightDelta
		});
	},
	...
	handleMove: function(ev) {
		...
		this.rightDelta = getEvX(ev) - this.x0;
...
var Grid = FC.Grid = Class.extend(ListenerMixin, MouseIgnorerMixin, {
	...
	// Given coordinates from the topright of the document, return data about the date-related area underneath.
	// Can return an object with arbitrary properties (although top/left/right/bottom are encouraged).
	// Must have a `grid` property, a reference to this current grid. TODO: avoid this
	// The returned object will be processed by getHitSpan and getHitEl.
	queryHit: function(rightOffset, topOffset) {
	},
...
var DayTableMixin = FC.DayTableMixin = {
	...
	dayDates: null, // whole-day dates for each column. right to left
	...
	// Intro means the rightmost cell when the calendar is LTR and the leftmost cell when RTL. Vice-versa for outro.
...
var DayGrid = FC.DayGrid = Grid.extend(DayTableMixin, {
	...
	spanToSegs: function(span) {
		...
		for (i = 0; i < segs.length; i++) {
			seg = segs[i];
			if (this.isRTL) {
				seg.rightCol = this.daysPerRow - 1 - seg.lastRowDayIndex;
				seg.leftCol = this.daysPerRow - 1 - seg.firstRowDayIndex;
			}
			else {
	...
	queryHit: function(rightOffset, topOffset) {
		if (this.colCoordCache.isRightInBounds(rightOffset) && this.rowCoordCache.isTopInBounds(topOffset)) {
			var col = this.colCoordCache.getHorizontalIndex(rightOffset);
	...
	/* Cell System
	------------------------------------------------------------------------------------------------------------------*/
	// FYI: the first column is the rightmost column, regardless of date
	...
	renderFillRow: function(type, seg, className) {
		var colCnt = this.colCnt;
		var startCol = seg.rightCol;
		var endCol = seg.leftCol + 1;
		var skeletonEl;
		var trEl;
...
DayGrid.mixin({
		...
		for (i = 0; i < levelCnt; i++) { // iterate through all levels
			...
			if (levelSegs) {
				for (j = 0; j < levelSegs.length; j++) { // iterate through segments in level
					...
					emptyCellsUntil(seg.rightCol);

					// create a container that occupies or more columns. append the event element.
					td = $('<td class="fc-event-container"/>').append(seg.el);
					if (seg.rightCol != seg.leftCol) {
						td.attr('colspan', seg.leftCol - seg.rightCol + 1);
					}
					else { // a single-column segment
						loneCellMatrix[i][col] = td;
					}

					while (col <= seg.leftCol) {
						cellMatrix[i][col] = td;
						segMatrix[i][col] = seg;
						col++;
					}
	...
	buildSegLevels: function(segs) {
		...
		// order segments right-to-left. very important if calendar is RTL
		for (j = 0; j < levels.length; j++) {
...
// A cmp function for determining the rightmost event
...
DayGrid.mixin({
	...
	limitRow: function(row, levelLimit) {
		...
		var col = 0; // col #, right-to-left (not chronologically)
		var levelSegs; // array of segment objects in the last allowable level, ordered right-to-left
		...
		if (levelLimit && levelLimit < rowStruct.segLevels.length) { // is it actually over the limit?
			...
			for (i = 0; i < levelSegs.length; i++) {
				seg = levelSegs[i];
				emptyCellsUntil(seg.rightCol); // process empty cells before the segment
				...
				if (totalSegsBelow) { // do we need to replace this segment with one or many "more" links?
					td = cellMatrix[levelLimit - 1][seg.rightCol]; // the segment's parent cell
	...
	showSegPopover: function(row, col, moreLink, segs) {
		...
		if (this.isRTL) {
			options.left = moreWrap.offset().right + moreWrap.outerWidth() + 1; // +1 to be over cell border
		}
		else {
			options.right = moreWrap.offset().right - 1; // -1 to be over cell border
		}
...
var TimeGrid = FC.TimeGrid = Grid.extend(DayTableMixin, {
	...
	queryHit: function(rightOffset, topOffset) {
		...
		if (colCoordCache.isRightInBounds(rightOffset) && slatCoordCache.isTopInBounds(topOffset)) {
			var colIndex = colCoordCache.getHorizontalIndex(rightOffset);
...
TimeGrid.mixin({
	...
	// from 0 to 1. If the calendar is right-to-left, the seg.backwardCoord maps to "right" and
	// seg.forwardCoord maps to "left" (via percentage). Vice-versa if the calendar is left-to-right.
	//
	...
	generateFgSegHorizontalCss: function(seg) {
		...
		var backwardCoord = seg.backwardCoord; // the right side if LTR. the left side if RTL. floating-point
		var forwardCoord = seg.forwardCoord; // the left side if LTR. the right side if RTL. floating-point
		...
		if (shouldOverlap && seg.forwardPressure) {
			...
			props[this.isRTL ? 'marginRight' : 'marginLeft'] = 10 * 2; // 10 is a guesstimate of the icon's width
		}
...
// Builds an array of segments "levels". The first level will be the rightmost tier of segments if the calendar is
// right-to-left, or the leftmost if the calendar is left-to-right. Assumes the segments are already ordered by date.
...
var Scroller = FC.Scroller = Class.extend({
	...
	lockOverflow: function(scrollbarWidths) {
		...
		if (overflowY === 'auto') {
			overflowY = (
					scrollbarWidths.right || scrollbarWidths.left || // vertical scrollbars?
					// OR scrolling pane with massless scrollbars?
	...
	function renderSection(position) {
		...
		if (buttonStr) {
			$.each(buttonStr.split(' '), function(i) {
				...
				if (isOnlyButtons) {
					groupChildren
						.first().addClass(tm + '-corner-right').end()
						.last().addClass(tm + '-corner-left').end();
				}
...
Calendar.rtlDefaults = { // left-to-right defaults
...
var BasicView = FC.BasicView = View.extend({
	...
	setHeight: function(totalHeight, isAuto) {
		...
		if (!isAuto) { // should we force dimensions of the scroll container?
			...
			if (scrollbarWidths.right || scrollbarWidths.left) { // using scrollbars?
	...
	queryHit: function(right, top) {
		return this.dayGrid.queryHit(right, top);
	},
...
var AgendaView = FC.AgendaView = View.extend({
	...
	queryHit: function(right, top) {
		var hit = this.timeGrid.queryHit(right, top);

		if (!hit && this.dayGrid) {
			hit = this.dayGrid.queryHit(right, top);
		}
