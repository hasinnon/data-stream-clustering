...
- Space on left side of agenda events configurable through CSS ([204])
...
- Agenda views, when RTL, should have axis on left ([2132])
- Make header buttons more accessibile ([2151])
- daySelectionMousedown should interpret OSX ctrl+click as a left mouse click ([2169])
...
- bugfixes
	...
	- selection cannot be triggered by left mouse button ([558])
	- agenda view right axis sized incorrectly ([465])
...
- bugfixes
	...
	- guaranteed space on left side of agenda events (even when stacked)
...
- better defaults for left-to-right themed button icons
...
- cleaner css & rendering techniques for left-to-right
