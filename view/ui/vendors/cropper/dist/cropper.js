 ...
 *
 * Copyleft (c) 2014-2016 Fengyuan Chen and contributors
...
(function (factory) {
  ...
  Cropper.prototype = {
    ...
    read: function (arrayBuffer) {
      ...
      if (orientation > 1) {
        ...
        switch (orientation) {
          ...
          // flip horizontal + rotate left 90°
          case 7:
            rotate = 90;
            scaleX = -1;
            break;
    ...
    preview: function () {
      ...
      var right = cropBox.right - canvas.right - image.right;
    ...
    change: function (shiftKey, event) {
      ...
      if (this.limited) {
        minRight = cropBox.minRight;
        minTop = cropBox.minTop;
        maxWidth = minRight + min(container.width, canvas.right + canvas.width);
      ...
      switch (action) {
        ...
        // Resize crop box
        case ACTION_EAST:
          if (range.x >= 0 && (left >= maxWidth || aspectRatio &&
        ...
        case ACTION_WEST:
          if (range.x <= 0 && (right <= minRight || aspectRatio &&
            (top <= minTop || bottom >= maxHeight))) {
        ...
        case ACTION_NORTH_EAST:
          if (aspectRatio) {
            if (range.y <= 0 && (top <= minTop || left >= maxWidth)) {
              renderable = false;
              break;
            }
        ...
        case ACTION_NORTH_WEST:
          if (aspectRatio) {
            if (range.y <= 0 && (top <= minTop || right <= minRight)) {
              renderable = false;
              break;
            }
        ...
        case ACTION_SOUTH_WEST:
          if (aspectRatio) {
            if (range.x <= 0 && (right <= minRight || bottom >= maxHeight)) {
              renderable = false;
              break;
            }
        ...
        case ACTION_SOUTH_EAST:
          if (aspectRatio) {
            if (range.x >= 0 && (left >= maxWidth || bottom >= maxHeight)) {
              renderable = false;
              break;
            }
        ...
        case ACTION_CROP:
          ...
          right = this.startX - offset.right;
    ...
    move: function (offsetX, offsetY) {
      ...
      this.moveTo(
        isUndefined(offsetX) ? offsetX : canvas.right + num(offsetX),
    ...
    zoomTo: function (ratio, _event) {
      ...
      if (ratio >= 0 && this.isBuilt && !this.isDisabled && options.zoomable) {
        ...
        if (originalEvent) {
          ...
          canvas.right -= (newWidth - width) * (
            ((center.pageX - offset.right) - canvas.right) / width
          );
    ...
    setData: function (data) {
      ...
      if (this.isBuilt && !this.isDisabled && $.isPlainObject(data)) {
        ...
        if (isNumber(data.x)) {
          cropBoxData.right = data.x * ratio + canvas.right;
        }
