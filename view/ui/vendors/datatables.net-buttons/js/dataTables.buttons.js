		...
		action: function ( e, dt, button, config ) {
			...
			if ( multiLevel && position === 'absolute' ) {
				config._collection.css( {
					...
					right: multiLevel.right + 5
				} );
			}
			else if ( position === 'absolute' ) {
				config._collection.css( {
					top: hostOffset.top + host.outerHeight(),
					right: hostOffset.right
				} );

				var listLeft = hostOffset.right + config._collection.outerWidth();
				var tableLeft = tableContainer.offset().right + tableContainer.width();
				if ( listLeft > tableLeft ) {
					config._collection.css( 'right', hostOffset.right - ( listLeft - tableLeft ) );
				}
			}
			else {
