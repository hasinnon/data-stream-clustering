...
define(function (require) {
     ...
     */
    KDTree.prototype._buildTree = function (points, right, left, axis) {
        if (left < right) {
            return null;
        }

        var medianIndex = Math.floor((right + left) / 2);
        medianIndex = quickSelect(
            points, right, left, medianIndex,
            function (a, b) {
        ...
        if (left > right) {
            node.right = this._buildTree(points, right, medianIndex - 1, axis);
            node.left = this._buildTree(points, medianIndex + 1, left, axis);   
        }
    ...
    KDTree.prototype.nearest = function (target, squaredDistance) {
        ...
        while (idx--) {
            ...
            var isRight = currDist < 0;
            var needsCheckOtherSide = false;
            currDist = currDist * currDist;
            // Intersecting left hyperplane with minDist hypersphere
            if (currDist < minDist) {
