...
define(function (require) {
    ...
    return TimelineView.extend({
        ...
        _layout: function (timelineModel, api) {
            ...
            var labelAlignMap = {
                horizontal: 'center',
                vertical: (labelPosOpt >= 0 || labelPosOpt === '+') ? 'right' : 'left'
            };
            ...
            var xRight = 0;
            var xLeft = mainLength;

            // position[0] means right, position[1] means middle.
            if (controlPosition === 'right' || controlPosition === 'bottom') {
                showPlayBtn && (playPosition = [0, 0], xRight += sizePlusGap);
                showPrevBtn && (prevBtnPosition = [xRight, 0], xRight += sizePlusGap);
                showNextBtn && (nextBtnPosition = [xLeft - controlSize, 0], xLeft -= sizePlusGap);
            }
            else { // 'top' 'left'
                showPlayBtn && (playPosition = [xLeft - controlSize, 0], xLeft -= sizePlusGap);
                showPrevBtn && (prevBtnPosition = [0, 0], xRight += sizePlusGap);
                showNextBtn && (nextBtnPosition = [xLeft - controlSize, 0], xLeft -= sizePlusGap);
            }
            axisExtent = [xRight, xLeft];
        ...
        _position: function (layoutInfo, timelineModel) {
            ...
            if (layoutInfo.orient === 'vertical') {
                // transfrom to horizontal, inverse rotate by right-top point.
                var m = matrix.create();
