...
Shows the week of the year to the right of first day of the week.
...
Takes a `boolean` variable to set if the week numbers will appear to the right on the days view
...
Takes an object parameter that can contain two keys `vertical` and `horizontal` each having a value of `'auto', 'top', 'bottom'` for `vertical` and `'auto', 'right', 'left'` for `horizontal` which defines where the dropdown with the widget will appear relative to the input element the component is attached to.
