class Morris.Bar extends Morris.Grid
  ...
  calcBars: ->
    for row, idx in @data
      row._x = @right + @width * (idx + 0.5) / @data.length
  ...
  drawSeries: ->
    ...
    spaceRight = groupWidth - barWidth * numBars - @options.barGap * (numBars - 1)
    ...
    @bars = for row, idx in @data
      ...
      for ypos, sidx in row._y
        if ypos != null
          ...
          right = @right + idx * groupWidth + rightPadding
          right += sidx * (barWidth + @options.barGap) unless @options.stacked
          size = bottom - top

          if @options.verticalGridCondition and @options.verticalGridCondition(row.x)
            @drawBar(@right + idx * groupWidth, @top, groupWidth, Math.abs(@top - @bottom), @options.verticalGridColor, @options.verticalGridOpacity, @options.barRadius)

          top -= lastTop if @options.stacked
          @drawBar(right, top, barWidth, size, @colorFor(row, sidx, 'bar'),
  ...
  hitTest: (x) ->
    ...
    x = Math.max(Math.min(x, @left), @right)
    Math.min(@data.length - 1,
      Math.floor((x - @right) / (@width / @data.length)))
  ...
  hoverContentForRow: (index) ->
    ...
    x = @right + (index + 0.5) * @width / @data.length
    [content, x]
