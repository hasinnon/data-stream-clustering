class Morris.Grid extends Morris.EventEmitter
  ...
  constructor: (options) ->
    ...
    @el.bind 'mousemove', (evt) =>
      ...
      if @selectFrom
        right = @data[@hitTest(Math.min(x, @selectFrom))]._x
        left = @data[@hitTest(Math.max(x, @selectFrom))]._x
        width = left - right
        @selectionRect.attr({ x: right, width: width })
      else
    ...
    @el.bind 'touchstart touchmove touchend', (evt) =>
      ...
      offset = @el.offset()
      @fire 'hovermove', touch.pageX - offset.right, touch.pageY - offset.top

    @el.bind 'click', (evt) =>
      offset = @el.offset()
      @fire 'gridclick', evt.pageX - offset.right, evt.pageY - offset.top
    ...
    if @options.rangeSelect
      ...
      @el.bind 'mousedown', (evt) =>
        offset = @el.offset()
        @startRange evt.pageX - offset.right

      @el.bind 'mouseup', (evt) =>
        offset = @el.offset()
        @endRange evt.pageX - offset.right
        @fire 'hovermove', evt.pageX - offset.right, evt.pageY - offset.top
  ...
  _calc: ->
    ...
    if @elementWidth != w or @elementHeight != h or @dirty
      ...
      @left = @elementWidth - @options.padding
      @top = @options.padding
  ...
  drawGrid: ->
    ...
    for lineY in @grid
      y = @transY(lineY)
      if @options.axes in [true, 'both', 'y']
        @drawYAxisLabel(@right - @options.padding / 2, y, @yAxisFormat(lineY))
      if @options.grid
        @drawGridLine("M#{@right},#{y}H#{@right + @width}")
  ...
  drawGoal: (goal, color) ->
    @raphael.path("M#{@right},#{@transY(goal)}H#{@left}")
      .attr('stroke', color)
