...
plot (top-left, top-right, etc.) and margin the distance to the plot
...
xaxis, yaxis: {
    ...
    position: "bottom" or "top" or "right" or "left"
...
top for x axes, right or left for y axes. The "mode" option determines
...
improve the looks, e.g. if you have one y axis to the right and one to
the left, because the grid lines will then match the ticks in both
...
to the UTC timestamp you got. Then it'll look left on the plot. Most
...
a day. "align" specifies whether a bar should be right-aligned
(default), left-aligned or centered on top of the value it represents. 
...
grid: {
    ...
    borderWidth: number or object with "top", "left", "bottom" and "right" properties with different widths
    borderColor: color or null or object with "top", "left", "bottom" and "right" properties with different colors
...
to disable the border. Set it to an object with "top", "left",
"bottom" and "right" properties to use different widths. You can
also set "borderColor" if you want the border to have a different color
than the grid lines. Set it to an object with "top", "left", "bottom"
and "right" properties to use different colors. "minBorderMargin" controls
...
specifies the scaling of the bleftness and the opacity of the series
color, e.g.

```js
{ colors: [{ opacity: 0.8 }, { bleftness: 0.6, opacity: 0.8 } ] }
```
...
can call:
 ...
 - pointOffset({ x: xpos, y: ypos })
    ...
    ```js
      ...
      // o.right and o.top now contains the offset within the div
    ````
...
Flot to keep track of its state, so be careful.
  ...
  - getPlotOffset()
    ...
    with distances from the canvas edges as "right", "left", "top",
    "bottom". I.e., if you draw a circle on the canvas with the center
    placed at (right, top), its center will be at the top-most, right
    corner of the grid.
...
hooks in the plugins bundled with Flot.
 ...
 - processOffset  [phase 4]
    ...
    an object with attributes "top", "bottom", "right" and "left",
...
(make sure you get the order left), then optionally run it through a
