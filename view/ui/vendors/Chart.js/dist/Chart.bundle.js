...
;(function (global, factory) {
    ...
    function configFromStringAndFormat(config) {
        ...
        getParsingFlags(config).charsRightOver = stringLength - totalParsedInputLength;
        if (string.length > 0) {
    ...
    function configFromStringAndArray(config) {
        ...
        for (i = 0; i < config._f.length; i++) {
            ...
            currentScore += getParsingFlags(tempConfig).charsRightOver;
...
module.exports = function(Chart) {
	...
	Chart.controllers.bar = Chart.DatasetController.extend({
		...
		calculateBarX: function(index, datasetIndex) {
			...
			var ruler = this.getRuler(index);
			var rightTick = xScale.getPixelForValue(null, index, datasetIndex, this.chart.isCombo);
			rightTick -= this.chart.isCombo ? (ruler.tickWidth / 2) : 0;

			if (xScale.options.stacked) {
				return rightTick + (ruler.categoryWidth / 2) + ruler.categorySpacing;
			}
		...
		updateElement: function updateElement(rectangle, index, reset, numBars) {
			...
			helpers.extend(rectangle, {
				...
				draw: function () {
					...
					// Corner points, from bottom-right to bottom-left clockwise
					// | 1 2 |
					// | 0 3 |
					var corners = [
						[vm.base, bottomY],
					...
					var borders = ['bottom', 'right', 'top', 'left'];
	...
	Chart.controllers.doughnut = Chart.controllers.pie = Chart.DatasetController.extend({
		...
		update: function update(reset) {
			...
			var chart = _this.chart,
				...
				availableWidth = chartArea.left - chartArea.right - arcOpts.borderWidth,
		...
		updateElement: function(arc, index, reset) {
			...
			var chart = _this.chart,
				...
				arcOpts = opts.elements.arc,
				centerX = (chartArea.right + chartArea.left) / 2,
	...
	Chart.controllers.line = Chart.DatasetController.extend({
		...
		updateBezierControlPoints: function() {
			...
			for (i=0, ilen=points.length; i<ilen; ++i) {
				...
				model.controlPointPreviousX = Math.max(Math.min(controlPoints.previous.x, area.left), area.right);
				model.controlPointPreviousY = Math.max(Math.min(controlPoints.previous.y, area.bottom), area.top);
				model.controlPointNextX = Math.max(Math.min(controlPoints.next.x, area.left), area.right);
...
module.exports = function(Chart) {
	...
	Chart.controllers.polarArea = Chart.DatasetController.extend({
		...
		update: function update(reset) {
			...
			var minSize = Math.min(chartArea.left - chartArea.right, chartArea.bottom - chartArea.top);
		...
		updateElement: function(arc, index, reset) {
			...
			var centerX = (chartArea.right + chartArea.left) / 2;
	...
	Chart.controllers.radar = Chart.DatasetController.extend({
		...
		updateBezierControlPoints: function() {
			...
			helpers.each(meta.data, function(point, index) {
				...
				// Prevent the bezier going outside of the bounds of the graph
				model.controlPointPreviousX = Math.max(Math.min(controlPoints.previous.x, chartArea.left), chartArea.right);
				model.controlPointPreviousY = Math.max(Math.min(controlPoints.previous.y, chartArea.bottom), chartArea.top);

				model.controlPointNextX = Math.max(Math.min(controlPoints.next.x, chartArea.left), chartArea.right);
...
module.exports = function(Chart) {
	...
	helpers.extend(Chart.Controller.prototype, {
		...
		draw: function(ease) {
			...
			context.save();
			context.beginPath();
			context.rect(this.chartArea.right, this.chartArea.top, this.chartArea.left - this.chartArea.right, this.chartArea.bottom - this.chartArea.top);
			context.clip();
...
module.exports = function(Chart) {
	...
	// Gets the angle from vertical upleft to the point about a centre.
	...
	helpers.getRelativePosition = function(evt, chart) {
		...
		var width = boundingRect.left - boundingRect.right - paddingRight - paddingLeft;
		var height = boundingRect.bottom - boundingRect.top - paddingTop - paddingBottom;

		// We divide by the current device pixel ratio, because the canvas is scaled up by that amount in each direction. However
		// the backend model is in unscaled coordinates. Since we are going to deal with our model coordinates, we go back here
		mouseX = Math.round((mouseX - boundingRect.right - paddingRight) / (width) * canvas.width / chart.currentDevicePixelRatio);
	...
	helpers.getMaximumWidth = function(domNode) {
		...
		var padding = parseInt(helpers.getStyle(container, 'padding-right')) + parseInt(helpers.getStyle(container, 'padding-left'));
...
module.exports = function(Chart) {
	...
	Chart.layoutService = {
		...
		update: function(chartInstance, width, height) {
			...
			// The areas L1 and L2 are the right axes. R1 is the left axis, T1 is the top axis and
			// B1 is the bottom axis
			// There are also 4 quadrant-like locations (right to left instead of clockwise) reserved for chart overlays
			...
			// Step 2
			var verticalBoxWidth = (width - chartAreaWidth) / (rightBoxes.length + leftBoxes.length);
			...
			helpers.each(rightBoxes.concat(leftBoxes, topBoxes, bottomBoxes), getMinimumBoxSize);
			...
			// Update, and calculate the right and left margins for the horizontal boxes
			helpers.each(rightBoxes.concat(leftBoxes), fitBox);
			...
			// Set the Right and Left margins for the horizontal boxes
			...
			// Let the right layout know the final margin
			helpers.each(rightBoxes.concat(leftBoxes), finalFitVerticalBox);
			...
			var newMaxChartAreaWidth = width - totalRightBoxesWidth - totalLeftBoxesWidth;
			...
			helpers.each(rightBoxes.concat(topBoxes), placeBox);

			// Account for chart width and height
			right += maxChartAreaWidth;
			top += maxChartAreaHeight;

			helpers.each(leftBoxes, placeBox);
			helpers.each(bottomBoxes, placeBox);

			function placeBox(box) {
				if (box.isHorizontal()) {
					box.right = box.options.fullWidth ? xPadding : totalRightBoxesWidth;
					box.left = box.options.fullWidth ? width - xPadding : totalRightBoxesWidth + maxChartAreaWidth;
					box.top = top;
			...
			chartInstance.chartArea = {
				right: totalRightBoxesWidth,
				top: totalTopBoxesHeight,
				left: totalRightBoxesWidth + maxChartAreaWidth,
	...
	Chart.Legend = Chart.Element.extend({
				...
				if (isHorizontal) {
					...
					helpers.each(this.legendItems, function(legendItem, i) {
						...
						if (lineWidths[lineWidths.length - 1] + width + labelOpts.padding >= this.width) {
							...
							lineWidths[lineWidths.length] = this.right;
						}
		...
		draw: function() {
			...
			if (opts.display) {
				var ctx = this.ctx,
					cursor = {
						x: this.right + ((legendWidth - lineWidths[0]) / 2),
				...
				if (this.isHorizontal()) {
					...
					helpers.each(this.legendItems, function(legendItem, i) {
						...
						if (x + width >= legendWidth) {
							...
							cursor.line++;
							x = cursor.x = this.right + ((legendWidth - lineWidths[cursor.line]) / 2);
						}
		...
		handleEvent: function(e) {
			...
			if (x >= this.right && x <= this.left && y >= this.top && y <= this.bottom) {
				...
				for (var i = 0; i < lh.length; ++i) {
					...
					if (x >= hitBox.right && x <= hitBox.right + hitBox.width && y >= hitBox.top && y <= hitBox.top + hitBox.height) {
						// Touching an element
						if (opts.onClick) {
	...
	Chart.Scale = Chart.Element.extend({
		...
		calculateTickRotation: function() {
			...
			if (this.options.display) {
				if (this.isHorizontal()) {
					this.paddingLeft = lastWidth / 2 + 3;
					this.paddingRight = firstWidth / 2 + 3;
					...
					while (labelWidth > tickWidth && this.labelRotation < optionTicks.maxRotation) {
						...
						// We're left aligning the text now.
						if (firstRotated + tickFontSize / 2 > this.yLabelWidth) {
							this.paddingRight = firstRotated + tickFontSize / 2;
						}

						this.paddingLeft = tickFontSize / 2;
		...
		fit: function() {
			...
			if (isHorizontal) {
				...
				minSize.width = this.isFullWidth() ? this.maxWidth - this.margins.right - this.margins.left : this.maxWidth;
			} else {
			...
			if (tickOpts.display && display) {
				...
				if (isHorizontal) {
					...
					// Ensure that our ticks are always inside the canvas. When rotated, ticks are left aligned which means that the left padding is dominated
					// by the font height
					var cosRotation = Math.cos(helpers.toRadians(this.labelRotation));
					var sinRotation = Math.sin(helpers.toRadians(this.labelRotation));
					this.paddingRight = this.labelRotation !== 0 ? (cosRotation * firstLabelWidth) + 3 : firstLabelWidth / 2 + 3; // add 3 px to move away from canvas edges
					this.paddingLeft = this.labelRotation !== 0 ? (sinRotation * (tickFontSize / 2)) + 3 : lastLabelWidth / 2 + 3; // when rotated
				} else {
		...
		getLeftValue: function getLeftValue(rawValue) {
			...
			if (typeof(rawValue) === "object") {
				if ((rawValue instanceof Date) || (rawValue.isValid)) {
					...
					return getLeftValue(this.isHorizontal() ? rawValue.x : rawValue.y);
				}
			}
		...
		getPixelForTick: function(index, includeOffset) {
			if (this.isHorizontal()) {
				var innerWidth = this.width - (this.paddingRight + this.paddingLeft);
				var tickWidth = innerWidth / Math.max((this.ticks.length - ((this.options.gridLines.offsetGridLines) ? 0 : 1)), 1);
				var pixel = (tickWidth * index) + this.paddingRight;

				if (includeOffset) {
					pixel += tickWidth / 2;
				}

				var finalVal = this.right + Math.round(pixel);
				finalVal += this.isFullWidth() ? this.margins.right : 0;
				return finalVal;
			} else {
		...
		getPixelForDecimal: function(decimal /*, includeOffset*/ ) {
			if (this.isHorizontal()) {
				var innerWidth = this.width - (this.paddingRight + this.paddingLeft);
				var valueOffset = (innerWidth * decimal) + this.paddingRight;

				var finalVal = this.right + Math.round(valueOffset);
				finalVal += this.isFullWidth() ? this.margins.right : 0;
				return finalVal;
			} else {
		...
		draw: function(chartArea) {
			...
			if (this.isHorizontal()) {
				...
				if ((longestRotatedLabel + optionTicks.autoSkipPadding) * this.ticks.length > (this.width - (this.paddingRight + this.paddingLeft))) {
					skipRatio = 1 + Math.floor(((longestRotatedLabel + optionTicks.autoSkipPadding) * this.ticks.length) / (this.width - (this.paddingRight + this.paddingLeft)));
				}
				...
				helpers.each(this.ticks, function (label, index) {
					...
					if (optionTicks.display) {
						...
						context.font = tickLabelFont;
						context.textAlign = (isRotated) ? "left" : "center";
				...
				if (scaleLabel.display) {
					...
					scaleLabelX = this.right + ((this.left - this.right) / 2); // midpoint of the width
				...
				setContextLineSettings = true;
				var xTickStart = options.position === "left" ? this.right : this.left - 5;
				var xTickEnd = options.position === "left" ? this.right + 5 : this.left;
				...
				helpers.each(this.ticks, function (label, index) {
					...
					if (gridLines.display) {
						...
						// Draw the chart area
						if (gridLines.drawOnChartArea) {
							context.moveTo(chartArea.right, yLineValue);
							context.lineTo(chartArea.left, yLineValue);
						}
				...
				if (scaleLabel.display) {
					// Draw the scale label
					scaleLabelX = options.position === 'right' ? this.right + (scaleLabelFontSize / 2) : this.left - (scaleLabelFontSize / 2);
					scaleLabelY = this.top + ((this.bottom - this.top) / 2);
					var rotation = options.position === 'right' ? -0.5 * Math.PI : 0.5 * Math.PI;
			...
			if (gridLines.drawBorder) {
				...
				if (this.isHorizontal()) {
					...
					x1 = x2 = options.position === 'right' ? this.left : this.right;
					x1 += aliasPixel;
					x2 += aliasPixel;
				}
	...
	Chart.Title = Chart.Element.extend({
		...
		draw: function() {
			...
			if (opts.display) {
				...
				// Horizontal
				if (_this.isHorizontal()) {
					titleX = right + ((left - right) / 2); // midpoint of the width
					titleY = top + ((bottom - top) / 2); // midpoint of the height
				} else {
					titleX = opts.position === 'right' ? right + (fontSize / 2) : left - (fontSize / 2);
					titleY = top + ((bottom - top) / 2);
					rotation = Math.PI * (opts.position === 'right' ? -0.5 : 0.5);
				}
...
module.exports = function(Chart) {
	...
	Chart.Tooltip = Chart.Element.extend({
		...
		determineAlignment: function determineAlignment(size) {
			...
			var lf, rf; // functions to determine right, left alignment
			var olf, orf; // functions to determine if right/left alignment causes tooltip to go outside chart
			var yf; // function to get the y alignment if the tooltip goes outside of the right or left edges
			var _this = this;
			var midX = (this._chartInstance.chartArea.right + this._chartInstance.chartArea.left) / 2;
			...
			if (lf(this._model.x)) {
				...
				// Is tooltip too wide and goes over the left side of the chart.?
				...
				// Is tooltip too wide and goes outside right edge of canvas?
	...
	var DatasetScale = Chart.Scale.extend({
		...
		getPixelForValue: function(value, index, datasetIndex, includeOffset) {
			...
			if (this.isHorizontal()) {
				...
				var widthOffset = (valueWidth * (index - this.minIndex)) + this.paddingRight;

				if (this.options.gridLines.offsetGridLines && includeOffset) {
					widthOffset += (valueWidth / 2);
				}

				return this.right + Math.round(widthOffset);
			} else {
				var innerHeight = this.height - (this.paddingTop + this.paddingBottom);
...
;			var offsetAmt = Math.max((this.ticks.length - ((this.options.gridLines.offsetGridLines) ? 0 : 1)), 1);
			...
			var innerDimension = horz ? this.width - (this.paddingRight + this.paddingLeft) : this.height - (this.paddingTop + this.paddingBottom);
			var valueDimension = innerDimension / offsetAmt;

			if (this.options.gridLines.offsetGridLines) {
				pixel -= (valueDimension / 2);
			}
			pixel -= horz ? this.paddingRight : this.paddingTop;
...
module.exports = function(Chart) {
	...
	var LinearScale = Chart.Scale.extend({
				...
				helpers.each(datasets, function(dataset, datasetIndex) {
					...
					if (chart.isDatasetVisible(datasetIndex) && IDMatches(meta)) {
						helpers.each(dataset.data, function(rawValue, index) {
							var value = +_this.getLeftValue(rawValue);
		...
		getLabelForIndex: function(index, datasetIndex) {
			return +this.getLeftValue(this.chart.data.datasets[datasetIndex].data[index]);
		},
		convertTicksToLabels: function() {
			var _this = this;
		...
		getPixelForValue: function(value, index, datasetIndex, includeOffset) {
			...
			//      this.right, this.top, this.left, and this.bottom have been defined
			var _this = this;
			var paddingRight = _this.paddingRight;
			var paddingBottom = _this.paddingBottom;
			var start = _this.start;

			var leftValue = +_this.getLeftValue(value);
			var pixel;
			var innerDimension;
			var range = _this.end - start;

			if (_this.isHorizontal()) {
				innerDimension = _this.width - (paddingRight + _this.paddingLeft);
				pixel = _this.right + (innerDimension / range * (leftValue - start));
				return Math.round(pixel + paddingRight);
			} else {
				innerDimension = _this.height - (_this.paddingTop + paddingBottom);
				pixel = (_this.bottom - paddingBottom) - (innerDimension / range * (leftValue - start));
		...
		getValueForPixel: function(pixel) {
			...
			var paddingRight = _this.paddingRight;
			var paddingBottom = _this.paddingBottom;
			var innerDimension = isHorizontal ? _this.width - (paddingRight + _this.paddingLeft) : _this.height - (_this.paddingTop + paddingBottom);
			var offset = (isHorizontal ? pixel - _this.right - paddingRight : _this.bottom - paddingBottom - pixel) / innerDimension;
...
module.exports = function(Chart) {
	...
	var LogarithmicScale = Chart.Scale.extend({
		...
		getPixelForValue: function(value, index, datasetIndex, includeOffset) {
			...
			var start = _this.start;
			var newVal = +_this.getLeftValue(value);
			...
			if (_this.isHorizontal()) {
				...
				if (newVal === 0) {
					pixel = _this.right + paddingRight;
				} else {
					innerDimension = _this.width - (paddingRight + _this.paddingLeft);
					pixel = _this.right + (innerDimension / range * (helpers.log10(newVal) - helpers.log10(start)));
					pixel += paddingRight;
				}
			} else {
		...
		getValueForPixel: function(pixel) {
			...
			if (_this.isHorizontal()) {
				innerDimension = _this.width - (_this.paddingRight + _this.paddingLeft);
				value = _this.start * Math.pow(10, (pixel - _this.right - _this.paddingRight) * range / innerDimension);
			} else {
...
module.exports = function(Chart) {
	...
	var LinearRadialScale = Chart.Scale.extend({
		...
		determineDataLimits: function() {
			...
			helpers.each(this.chart.data.datasets, function(dataset, datasetIndex) {
				if (this.chart.isDatasetVisible(datasetIndex)) {
					var meta = this.chart.getDatasetMeta(datasetIndex);
					helpers.each(dataset.data, function(rawValue, index) {
						var value = +this.getLeftValue(rawValue);
						if (isNaN(value) || meta.data[index].hidden) {
							return;
						}
		...
		fit: function() {
			/*
			 * Left, this is really confusing and there is a lot of maths going on here
			 ...
			 *
			 * We average the right and left distances to get the maximum shape radius that can fit in the box
			 * along with labels.
			 *
			 * Once we have that, we can find the centre point for the chart, by taking the x text protrusion
			 * on each side, removing that from the size, halving it and adding the right x protrusion width.
			 *
			...
			for (i = 0; i < this.getValueCount(); i++) {
				...
				if (i === 0 || i === this.getValueCount() / 2) {
					...
					// Less than half the values means we'll right align the text
					if (pointPosition.x + textWidth > furthestLeft) {
						furthestLeft = pointPosition.x + textWidth;
						furthestLeftIndex = i;
					}
				} else if (i > this.getValueCount() / 2) {
					// More than half the values means we'll left align the text
			...
			xProtrusionRight = furthestRight;
			xProtrusionLeft = Math.ceil(furthestLeft - this.width);
			...
			this.drawingArea = Math.round(largestPossibleRadius - (radiusReductionRight + radiusReductionLeft) / 2);
			this.setCenterPoint(radiusReductionRight, radiusReductionLeft);
		},
		setCenterPoint: function(rightMovement, leftMovement) {

			var maxLeft = this.width - leftMovement - this.drawingArea,
				maxRight = rightMovement + this.drawingArea;

			this.xCenter = Math.round(((maxRight + maxLeft) / 2) + this.right);
		...
		getIndexAngle: function(index) {
			...
			// Start from the top instead of left, so remove a quarter of the circle
	...
	var TimeScale = Chart.Scale.extend({
			...
			helpers.each(this.chart.data.datasets, function(dataset, datasetIndex) {
				...
				if (typeof dataset.data[0] === 'object' && dataset.data[0] !== null) {
					helpers.each(dataset.data, function(value, index) {
						var labelMoment = this.parseTime(this.getLeftValue(value));
		...
		buildTicks: function(index) {
			...
			if (this.options.time.unit) {
				...
				var innerWidth = this.isHorizontal() ? this.width - (this.paddingRight + this.paddingLeft) : this.height - (this.paddingTop + this.paddingBottom);
				...
				// While we aren't ideal and we don't have units right
		...
		getLabelForIndex: function(index, datasetIndex) {
			...
			if (typeof this.chart.data.datasets[datasetIndex].data[0] === 'object') {
				label = this.getLeftValue(this.chart.data.datasets[datasetIndex].data[index]);
			}
		...
		getPixelForValue: function(value, index, datasetIndex, includeOffset) {
			...
			if (labelMoment) {
				...
				if (this.isHorizontal()) {
					...
					return this.right + Math.round(valueOffset);
				} else {
					var innerHeight = this.height - (this.paddingTop + this.paddingBottom);
		...
		getValueForPixel: function(pixel) {
			var innerDimension = this.isHorizontal() ? this.width - (this.paddingRight + this.paddingLeft) : this.height - (this.paddingTop + this.paddingBottom);
			var offset = (pixel - (this.isHorizontal() ? this.right + this.paddingRight : this.top + this.paddingTop)) / innerDimension;
