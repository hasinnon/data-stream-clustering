...
```javascript
{
	right: Number, // right edge of the scale bounding box
	left: Number, // left edge of the bounding box'
	top: Number, 
	bottom: Number,
	width: Number, // the same as left - right
...
```javascript
	...
	// Draws the scale onto the canvas. this.(right|left|top|bottom) will have been populated to tell you the area on the canvas to draw in
	// @param chartArea : an object containing four properties: right, left, top, bottom. This is the rectangle that lines, bars, etc will be drawn in. It may be used, for example, to draw grid lines.
...
{	
	...
	getLeftValue: function(dataValue) {},
}
```
