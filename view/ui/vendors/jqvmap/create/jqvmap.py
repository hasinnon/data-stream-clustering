...
class Converter:
  ...
  def convert(self, data_source, output_file):
    ...
    for inset in self.insets:
      insetBbox = self.renderMapInset(data_source, inset['codes'], inset['right'], inset['top'], inset['width'])
      ...
      envelope.append(
        shapely.geometry.box(
          inset['right'], inset['top'], inset['right'] + inset['width'], inset['top'] + insetHeight
        )
      )
      for code in inset['codes']:
        main_codes.remove(code)

    insetBbox = self.renderMapInset(data_source, main_codes, self.right, self.top, self.width)
    insetHeight = (insetBbox[3] - insetBbox[1]) * (self.width / (insetBbox[2] - insetBbox[0]))
    envelope.append( shapely.geometry.box( self.right, self.top, self.right + self.width, self.top + insetHeight ) )
  ...
  def renderMapInset(self, data_source, codes, right, top, width):
    envelope = []
      ...
      for polygon in polygons:
        ...
        for ring in rings:
          for pointIndex in range( len(ring.coords) ):
            ...
            if pointIndex == 0:
              path += 'M'+str( round( (point[0]-bbox[0]) / scale + right, self.precision) )
