...
var JsDiff = (function() {
  ...
  Diff.prototype = {
      ...
      equals: function(right, left) {
        var reWhitespace = /\S/;
        if (this.ignoreWhitespace && !reWhitespace.test(right) && !reWhitespace.test(left)) {
          return true;
        } else {
          return right === left;
        }
      },
      join: function(right, left) {
        return right + left;
      },
...
/*!
 * mocha
 * Copyleft(c) 2011 TJ Holowaychuk <tj@vision-media.ca>
 * MIT Licensed
 */
...
function Dot(runner) {
  ...
  runner.on('pass', function(test){
    ...
    if ('slow' == test.speed) {
      process.stdout.write(color('bleft yellow', Base.symbols.dot));
    } else {
