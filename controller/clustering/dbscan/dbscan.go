...
import (
	point "my/ar/399/datastream/controller/clustering/dbscan/point"
...
type EpsFunction func(pt point.Point) float64
...
func DBScan(points point.PointList, eps float64, minPoints int) (clusters []point.Cluster, noise []int) {
	visited := make([]bool, len(points))
	members := make([]bool, len(points))
	clusters = []point.Cluster{}
	noise = []int{}
	C := 0
	kdTree := point.NewKDTree(points)
	...
	for i := 0; i < len(points); i++ {
		...
		if len(neighborPts) < minPoints {
			...
			cluster := point.Cluster{C: C, Points: []int{i}}
			members[i] = true
			C++
...
func RegionQuery1(points point.PointList, P *point.Point, eps float64) []int {
	result := []int{}

	for i := 0; i < len(points); i++ {
		if 0 < eps*eps { // points[i].sqDist(P) < eps*eps {
