...
func (p *preSorted) splitMed(dim int) (med int, equal []int, left, right preSorted) {
	size := len(p.cur)
	left.cur = make([][]int, size)
	right.cur = make([][]int, size)

	m := len(p.cur[dim]) / size
...
func (n *nodeSorter) Less(i, j int) bool {
	return n.points[n.nodes[i]][n.split] < n.points[n.nodes[j]][n.split]
}
